...
public class Page_History extends Page_Main {
    ...
    final Font fontTitle = new Font("\u6977\u4F53", 1, 20);
    FontMetrics fm;
    MenuItem menuQuote;
    MenuItem menuMarket;
    MenuItem menuKLine;
    ...
    public Page_History(Rectangle _rc, HQApplet applet) {
        ...
        m_iHighlightRow = 0;
        m_iHighlightCol = 0;
        super.m_applet.iCurrentPage = 6;
        m_vCode = new Vector();
        for(int i = 0; i < super.m_applet.m_codeList.size(); i++) {
            CodeTable codeTable = (CodeTable)super.m_applet.m_htProduct.get(super.m_applet.m_codeList.elementAt(i));
            if(codeTable != null && (codeTable.status == 1 || codeTable.status == 6))
                m_vCode.addElement(super.m_applet.m_codeList.elementAt(i));
        }
    ...
    void paintTitle(Graphics g) {
        ...
        String strTitle = super.m_applet.getShowString("History");
    ...
    private void paintPromptMessage(Graphics g) {
        String prompt = super.m_applet.getShowString("HistoryPrompt");
    ...
    void paintProduct(Graphics g) {
        String strText = super.m_applet.getShowString("PagePrefix") + (m_iCurPage + 1) + super.m_applet.getShowString("PageSuffix") + " " + super.m_applet.getShowString("TotalPagePrefix") + m_iTotalPage + super.m_applet.getShowString("TotalPageSuffix");
        g.setColor(HQApplet.rhColor.clGrid);
        ...
        for(int i = 0; i < m_iRows; i++) {
            for(int j = 0; j < m_iCols; j++) {
                if(iIndex >= m_vCode.size())
                    break;
                CodeTable codeTable = (CodeTable)super.m_applet.m_htProduct.get(m_vCode.elementAt(iIndex));
    ...
    boolean KeyPressed(KeyEvent e) {
        ...
        case 10: // '\n'
            ...
            if(iIndex < m_vCode.size())
                super.m_applet.showPageKLine((String)m_vCode.elementAt(iIndex));
            break;
        }
        return false;
    }
    ...
    void makeMenus() {
        ...
        menuKLine = new MenuItem(super.m_applet.getShowString("Analysis") + "  F5");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(cmd.indexOf("cmd_") >= 0)
            super.m_applet.UserCommand(cmd.substring(4));
        else
        if(cmd.equals("kline")) {
            int iIndex = m_iCurPage * (m_iRows * m_iCols) + m_iHighlightRow * m_iCols + m_iHighlightCol;
            if(iIndex < m_vCode.size())
