...
public class Reserve extends IndicatorBase {
    ...
    public Reserve(IndicatorPos pos, int Precision) {
        super(pos, Precision);
        super.m_strIndicatorName = "ORDER";
    ...
    private void GetMaxMin() {
        ...
        for(int i = super.m_pos.m_Begin; i <= super.m_pos.m_End; i++)
            if((float)super.m_kData[i].reserveCount > super.m_max)
                super.m_max = super.m_kData[i].reserveCount;
    ...
    private void DrawVolume(Graphics g) {
        ...
        for(int i = super.m_pos.m_Begin; i <= super.m_pos.m_End; i++) {
            int value = super.m_rc.y + super.m_iTextH + (int)((super.m_max - (float)super.m_kData[i].reserveCount) / valuey);
    ...
    private void AverageVolume(int iIndex) {
        ...
        double sum = 0.0D;
        for(int i = 0; i < n - 1; i++)
            sum += super.m_kData[i].reserveCount;

        for(int i = n - 1; i < super.m_kData.length; i++) {
            sum -= prevolume;
            sum += super.m_kData[i].reserveCount;
            data[i] = (float)(sum / (double)n);
            prevolume = super.m_kData[(i - n) + 1].reserveCount;
        }
